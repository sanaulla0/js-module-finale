...
let passbtn = document.getElementById('instockbtn');
let classbtn = document.getElementById('cbtn');
...
increasebtn.addEventListener('click',increasesort);
decreasebtn.addEventListener('click',decreasesort);
marksbtn.addEventListener('click',markssort);
 passbtn.addEventListener('click',pass);
 classbtn.addEventListener('click',searchforclass)
 genderbtn.addEventListener('click',genderfun)
searchbtn.addEventListener('click',searchme);

function reander(data){
   table.innerHTML = '';
     ...
     data.map((item)=>{
          ...
          <td>  ${item.id}  </td> 
          <td> <img src="${item.img_src}" height="40">
            ${item.first_name}
             ${item.last_name}  </td> 
          <td> ${item.gender}  </td> 
          <td> ${item.class}  </td> 
...
function increasesort(){
 let increasedata = data.sort((a,b)=>{
      if(a.first_name<b.first_name) return-1;
      else if(a.first_name>b.first_name) return 1;
      else return 0;
 }); 
       reander(increasedata);


}
  function decreasesort(){
     let decreasedata= data.sort((a,b)=>{
          if(a.first_name>b.first_name) return -1;
          else if(a.first_name<b.first_name) return 1;
          else return 0;

     });
               reander(decreasedata);
 ...
 function markssort(){
  ...
  function pass(){
   let stockdata =  data.filter((item)=>{
...
function searchme(){
 ...
 return item.email.toLowerCase().includes(a.toLowerCase()) || item.first_name.toLowerCase().includes(a.toLowerCase()) || item.last_name.toLowerCase().includes(a.toLowerCase);
                  
     })
        reander(searchproduct);
